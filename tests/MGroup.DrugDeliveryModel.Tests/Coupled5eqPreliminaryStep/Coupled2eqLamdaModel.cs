...
{
    public class Coupled2eqLamdaModel
    {
        ...
        public Coupled2eqLamdaModel( Eq9ModelProviderForStaggeredSolutionEx7Ref eq9ModelProvider, DistributedOdeModelBuilder distributedOdeModelProvider,
                ComsolMeshReader comsolReader,  Dictionary<int, double> lambda, double timeStep, double totalTime, int incrementsPerStep)
        {
        ...
        public void CreateModel(IParentAnalyzer[] analyzers, ISolver[] solvers)
        {
            ...
            model[0] = DistributedOdeModelProvider.GetModel();
            DistributedOdeModelProvider.AddBoundaryConditions(model[0]);
            if (CurrentTimeStep == 0)
            {
                DistributedOdeModelProvider.AddInitialConditionsForTheRestOfBulkNodes(model[0]);

            }
            (analyzers[0], solvers[0], nlAnalyzers[0]) = DistributedOdeModelProvider.GetAppropriateSolverAnalyzerAndLog(model[0], timeStep, totalTime, CurrentTimeStep, incrementsPerStep);
        ...
        public void CreateModelFirstTime(IParentAnalyzer[] analyzers, ISolver[] solvers)
        {
            if (!(CurrentTimeStep == 0))
            {
                DistributedOdeModelProvider.RetrieveLambdaSolution(ParentSolvers[0], NLAnalyzers[0], model[0], lambda);
