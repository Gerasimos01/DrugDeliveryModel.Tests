...
namespace MGroup.DrugDeliveryModel.Tests.Coupled5eqPreliminaryStep
{
    public class Coupled5eqDummyLamdaTSolution
    {
        const double Sc = 0.1;
        ...
        #region CoxBCs
        List<(BC, ConvectionDiffusionDof[], double[][], double[])> convectionDiffusionDirichletBC = new List<(BC, ConvectionDiffusionDof[], double[][], double[])>()
            {
                (BC.TopDirichlet, new ConvectionDiffusionDof[] { ConvectionDiffusionDof.UnknownVariable }, new double[1][]{new double[3] {0.1, 0.1, 0.1}}, new double[] {0.2}),
                (BC.RightDirichlet,new ConvectionDiffusionDof[] { ConvectionDiffusionDof.UnknownVariable }, new double[1][]{new double[3] {0.1, 0.1, 0.1}}, new double[] {0.2d}),
                (BC.BackDirichlet, new ConvectionDiffusionDof[] { ConvectionDiffusionDof.UnknownVariable }, new double[1][]{new double[3] {0.1, 0.1, 0.1}}, new double[] {0.2d}),
            };
        List<(BC, ConvectionDiffusionDof[], double[][], double[])> convectionDiffusionNeumannBC = new List<(BC, ConvectionDiffusionDof[], double[][], double[])>();
        #endregion
        ...
        public Coupled5eqDummyLamdaTSolution()
        {
        ...
        public void MonophasicEquationModel(string fileName, double diffusionFactor)
        {
            ...
            structuralMonitorID = FindNodeIdFromNodalCoordinates(comsolReader.NodesDictionary, structuralMonitorNodeCoords, 1e-2);
            ...
            pressureMonitorID = FindNodeIdFromNodalCoordinates(comsolReader.NodesDictionary, pressureMonitorNodeCoords, 1e-2);
            ...
            var equationModel = new Coupled5eqDummyLamdaTModel(eq78Model, coxModel, eq9Model, tCellModel, distributedOdeModel, comsolReader,
            ...
            True(ResultChecker.CheckResults(gp_dP_dy_OverTime, expected_dpdy_values(), 1e-3));
            True(ResultChecker.CheckResults(gp_dP_dz_Overtime, expected_dpdz_values(), 1e-3));
            True(ResultChecker.CheckResults(p_i, expectedPressurevalues(), 1e-3));
            True(ResultChecker.CheckResults(coxResults, expectedCoxForDiffusion1000X(), 1e-3)); // and of course staggerd solvers norms -1 


            True(ResultChecker.CheckResults(solidVelocityResultsX, expectedSolidVelocityX(), 1e-3));
            True(ResultChecker.CheckResults(solidVelocityResultsY, expectedSolidVelocityY(), 1e-3));
            True(ResultChecker.CheckResults(solidVelocityResultsZ, expectedSolidVelocityZ(), 1e-3));

            True(ResultChecker.CheckResults(gp_dut_dx_OverTime, expected_dutdx_values(), 1e-3));
            True(ResultChecker.CheckResults(gp_dvt_dy_OverTime, expected_dvtdy_values(), 1e-3));
            True(ResultChecker.CheckResults(gp_dwt_dz_OverTime, expected_dwtdz_values(), 1e-3));
            True(ResultChecker.CheckResults(gp_div_v_OverTime, expected_div_vs_values(), 1e-1));
